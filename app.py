...
import datetime
from resume_filter.filter import render_resume_filter
...
def load_lottie_url(url: str):
    """Load Lottie animation JSON (cached to avoid reloads)"""
    r = requests.get(url)
...
class ResumeApp:
    def __init__(self):
        ...
        self.pages = {
            ...
            "🔽 RESUME Filter": self.render_resume_filter,      
            "🎯 Employee Attrition": self.render_employee_attrition,             
    ...
    def add_footer(self):
        ...
        with col2:
            ...
            <div style='display: flex; justify-content: center; align-items: center; margin-bottom: 10px;'>
                <a href='https://github.com/Juee504' target='_blank' style='text-decoration: none;'>
            ...
            <p style='text-align: center;'>
                 Developed by 
                <a href="https://www.linkedin.com/in/juee-jamsandekar-243848239/" target="_blank" style='text-decoration: none; color: #FFFFFF'>
                    <b>Juee Jamsandekar</b>
                </a>
            </p>
            <p style='text-align: center; font-size: 15px; color: #888888;'>
                "Streamlining hiring with intelligent resume analysis, JD matching, and real-time 
recruiter insights."
            </p>
            """, unsafe_allow_html=True)
    ...
    def render_builder(self):
            ...
            # Title with image icon and text
            st.markdown("""
                <div style='display: flex; align-items: center; gap: 10px;'>
                    <img src='https://cdn-icons-png.flaticon.com/512/3135/3135715.png' width='50'/>
                    <h1 style='color: #5AC8FA; font-family: Arial; margin: 0;'>Resume Builder</h1>
                </div>
            """, unsafe_allow_html=True)

            st.markdown("""
                <style>
                        .st-emotion-cache-9ajs8n p{ font-size: 20px !important;}
        ...
        "Position *", key=f"position_{idx}", value=exp.get(
            'position', ''), placeholder="Job Title")
                    with col2:
                                                
                        # --- Start Date Picker ---
                        start_date = st.date_input(
                            "Start Date * (pick any day of the month)",
                            value=datetime.date.today(),
                            max_value=datetime.date.today(),  # Cannot select future start date
                            key=f"start_date_{idx}"
                        )
                        exp['start_date'] = start_date.strftime("%m/%Y")

                        # --- End Date Picker / Optional 'Present' ---
                        end_date_option = st.radio(
                            "End Date",
                            options=["Select Date", "Present"],
                            key=f"end_date_option_{idx}"
                        )

                        if end_date_option == "Present":
                            exp['end_date'] = "Present"
                        else:
                            end_date = st.date_input(
                                "End Date (pick any day of the month)",
                                value=datetime.date.today(),
                                max_value=datetime.date.today(),  # Cannot select future end date
                                key=f"end_date_picker_{idx}"
                            )
                            exp['end_date'] = end_date.strftime("%m/%Y")
        ...
        placeholder="Project Name")
                    ...
                    proj_resp_text = st.text_area("Enter responsibilities (one per line)",
                                                ...
                                                placeholder="Designed system architecture\nImplemented core features\nTested and deployed application")
        ...
        "Degree *", key=f"degree_{idx}", value=edu.get(
            'degree', ''), placeholder="Bachelor of Science")
                    with col2:
        ...
        placeholder="Computer Science")
                        # Graduation date picker (no pre-filled value)
                    ...
                    # Store as MM/YYYY
                    edu['graduation_date'] = grad_date.strftime("%m/%Y")
        ...
        st.session_state.form_data['skills_categories']['tools']),
            ...
            if st.button("Generate Resume 📄", type="primary"):
                ...
                if linkedin:
                    if not (linkedin.startswith("https://www.linkedin.com/") or linkedin.startswith("http://www.linkedin.com/")):
                        errors.append("LinkedIn URL must be a valid LinkedIn profile URL starting with https://www.linkedin.com/")

                # Validate Portfolio URL if provided
                if portfolio and not (portfolio.startswith("http://") or portfolio.startswith("https://")):
                ...
                if not summary.strip():
                    ...
                    errors.append("Professional Summary should be at least 50 characters")
                
                # Experience Validation
                for idx, exp in enumerate(st.session_state.form_data['experiences']):
                    if not exp.get('company', '').strip():
                        errors.append(f"Company name is required for Experience {idx + 1}")
                    if not exp.get('position', '').strip():
                        errors.append(f"Position is required for Experience {idx + 1}")
                    if not exp.get('start_date', '').strip():
                        errors.append(f"Start date is required for Experience {idx + 1}")
                
                # Projects Validation
                for idx, proj in enumerate(st.session_state.form_data['projects']):
                    if not proj.get('name', '').strip():
                        errors.append(f"Project name is required for Project {idx + 1}")
                    if not proj.get('technologies', '').strip():
                        errors.append(f"Technologies are required for Project {idx + 1}")
                    if not proj.get('description', '').strip():
                        errors.append(f"Project description is required for Project {idx + 1}")
                
                # Education Validation
                for idx, edu in enumerate(st.session_state.form_data['education']):
                    if not edu.get('school', '').strip():
                        errors.append(f"School/University is required for Education {idx + 1}")
                    if not edu.get('degree', '').strip():
                        errors.append(f"Degree is required for Education {idx + 1}")
                    if not edu.get('field', '').strip():
                        errors.append(f"Field of study is required for Education {idx + 1}")
                    if not edu.get('graduation_date', '').strip():
                        errors.append(f"Graduation date is required for Education {idx + 1}")
                
                # Skills Validation
                if not tech_skills.strip():
                    errors.append("At least one Technical Skill is required")
                if not soft_skills.strip():
                    errors.append("At least one Soft Skill is required")
                if not languages.strip():
                    errors.append("At least one Language is required")
                if not tools.strip():
                    errors.append("At least one Tool/Technology is required")
                ...
                # If no errors, proceed with resume generation
                try:
                    ...
                    resume_data = {
                        ...
                        "skills": st.session_state.form_data.get('skills_categories', {
                            'technical': [s.strip() for s in tech_skills.split('\n') if s.strip()],
     ...
     "")
        ...
        st.markdown(f"""
            <div class="profile-section">
                <h2 class="profile-name">Juee Jamsandekar</h2>
                <p class="profile-title">Full Stack Developer & AI/ML Enthusiast</p>
                <div class="social-links">
                    <a href="https://github.com/Juee504" class="social-link" target="_blank">
                        <i class="fab fa-github"></i>
                    </a>
                    <a href="https://www.linkedin.com/in/juee-jamsandekar-243848239/" class="social-link" target="_blank">
                        <i class="fab fa-linkedin"></i>
                    </a>
                    <a href="mailto:jueejamsandekar54@gmail.com" class="social-link" target="_blank">
    ...
    def render_analyzer(self):
        ...
        st.markdown("""
            <style>
                    .st-emotion-cache-17r1dd6 p{ font-size: 20px !important;}
    ...
    def render_resume_filter(self):
        """Render the job search page"""
        render_resume_filter()

    def render_employee_attrition(self):
        """Render the employee_attrition page"""
...
<div style="background-color: #1e1e1e; border-radius: 10px; border: 1px solid #4b6cb7; padding: 10px; margin: 10px 0; color: white;">
    <div style="margin-bottom: 10px;">🚀Explore these awesome tools/repositories/resources:</div>
    <div style="margin-bottom: 5px;"><b>Freshers & Students:</b></div>
    <ul style="margin-top: 0; padding-left: 20px;">
        <li><a href="https://leetcode.com/" target="_blank" style="color: #4CAF50;">LeetCode</a></li>
        <li><a href="https://www.hackerrank.com/" target="_blank" style="color: #4CAF50;">HackerRank</a></li>
        <li><a href="https://www.geeksforgeeks.org/" target="_blank" style="color: #4CAF50;">GeeksforGeeks</a></li>
        <li><a href="https://www.kaggle.com/" target="_blank" style="color: #4CAF50;">Kaggle</a></li>
        <li><a href="https://www.fast.ai/" target="_blank" style="color: #4CAF50;">fast.ai Courses</a></li>
        <li><a href="https://www.kaggle.com/pavansubhasht/ibm-hr-analytics-attrition-dataset" target="_blank" style="color: #4CAF50;">IBM HR Analytics Dataset</a></li>
        <li><a href="https://www.coursera.org/professional-certificates/google-data-analytics" target="_blank" style="color: #4CAF50;">Google Data Analytics Certificate</a></li>
        <li><a href="https://learn.microsoft.com/en-us/certifications/azure-ai-fundamentals/" target="_blank" style="color: #4CAF50;">Microsoft Azure AI Fundamentals</a></li>
        <li><a href="https://aws.amazon.com/certification/certified-machine-learning-specialty/" target="_blank" style="color: #4CAF50;">AWS Machine Learning Specialty</a></li>
    </ul>
    <div style="margin-bottom: 5px;"><b>Professionals:</b></div>
    <ul style="margin-top: 0; padding-left: 20px;">
        <li><a href="https://www.coursera.org/learn/wharton-people-analytics" target="_blank" style="color: #4CAF50;">People Analytics by Wharton (Coursera)</a></li>
        <li><a href="https://executive.mit.edu/course/people-analytics" target="_blank" style="color: #4CAF50;">MIT Sloan – People Analytics</a></li>
        <li><a href="https://www.coursera.org/courses?query=hr%20analytics&utm_source" target="_blank" style="color: #4CAF50;">Advanced HR Analytics – LinkedIn Learning</a></li>
        <li><a href="https://github.com/topics/machine-learning-project" target="_blank" style="color: #4CAF50;">End-to-End ML Projects – GitHub</a></li>
        <li><a href="https://www.youtube.com/@krishnaik06" target="_blank" style="color: #4CAF50;">Full-Stack Data Science Projects by Krish Naik</a></li>
        <li><a href="https://cloud.google.com/certification/machine-learning-engineer" target="_blank" style="color: #4CAF50;">Google Cloud Professional ML Engineer</a></li>
    </ul>
    <div style="margin-bottom: 5px;"><b>Recruiters & Hiring Managers:</b></div>
    <ul style="margin-top: 0; padding-left: 20px;">
        <li><a href="https://www.hirevue.com/" target="_blank" style="color: #4CAF50;">HireVue</a></li>
        <li><a href="https://www.pymetrics.com/" target="_blank" style="color: #4CAF50;">Pymetrics</a></li>
        <li><a href="https://business.linkedin.com/talent-solutions/talent-insights" target="_blank" style="color: #4CAF50;">LinkedIn Talent Insights</a></li>
        <li><a href="https://www.shrm.org/" target="_blank" style="color: #4CAF50;">SHRM</a></li>
        <li><a href="https://www.glassdoor.com/employers/" target="_blank" style="color: #4CAF50;">Glassdoor for Employers</a></li>
        <li><a href="https://www.mckinsey.com/business-functions/organization/our-insights" target="_blank" style="color: #4CAF50;">McKinsey HR Reports</a></li>
        <li><a href="https://www.linkedin.com/learning/topics/recruiter" target="_blank" style="color: #4CAF50;">
LinkedIn Recruiter Online Training Courses
</a></li>
        <li><a href="https://www.aihr.com/courses/people-analytics-certificate/" target="_blank" style="color: #4CAF50;">AIHR Digital HR & People Analytics</a></li>
        <li><a href="https://www.shrm.org/in/credentials/certification" target="_blank" style="color: #4CAF50;">SHRM-CP Certification</a></li>
    </ul>
    <div style="margin-top: 10px;">⭐Explore these resources and level up your skills!</div>
</div>
"""
    ...
    def main(self):
        ...
        with st.sidebar:
            ...
            # st.markdown("""
            # <div style="min-height: 220px; position: relative; margin-bottom: 20px;">
            # """, unsafe_allow_html=True)
            
            # lottie_animation = load_lottie_url("https://assets5.lottiefiles.com/packages/lf20_xyadoh9h.json")
